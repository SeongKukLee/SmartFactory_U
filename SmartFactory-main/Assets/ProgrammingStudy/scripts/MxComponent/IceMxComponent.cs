    ...
    IEnumerator GetDevices()
    {
        while (true)
        {
            if (connection == Connection.Connected) //연결이 되어있을 때 GetDevice로 데이터를 받아오겠다. 
            {
                ...
                cylinderC.plcForwardValue = GetDevice(cylinderC.plcDeviceForwardAddress); // Y4의 값을 불러온다. 
                cylinderC.plcBackwardValue = GetDevice(cylinderC.plcDeviceBackwardAddress); // Y5의 값을 불러온다
